...
c_ch = 0
with open("out-predictions/GBC_predictions.csv", "wb") as csvfile_with_pred1:
    csv_out = csv.writer(csvfile_with_pred1)
    print "* Write: " + "GBC_predictions.csv" + " - Start - ",
...
with open("out-predictions/GBC_predictions.csv", "r") as impr_pred_file:
    csv_read = csv.reader(impr_pred_file)
    print "* READ:" + "GBC_predictions.csv"
    for i in range(5):
        print "     ", csv_read.next()
